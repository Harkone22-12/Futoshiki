...
def heuristic_mac(domains, n):
    """Hàm Heuristic mới: Đếm số lượng ô chưa được chốt (domain > 1)"""
    return sum(1 for r in range(n) for c in range(n) if len(domains[r][c]) > 1)
...
if __name__ == "__main__":
    ...
    solution_grid, nodes_expanded = solve_astar_mac_mrc(env)
